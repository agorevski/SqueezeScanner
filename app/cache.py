...
from .scanner import MarketDataProvider, TickerSnapshot
...
class CachedMarketDataProvider:
    ...
    def recent_snapshots(self, max_age_seconds: int | None = None) -> list[TickerSnapshot]:
        ...
        for row in rows:
            try:
                snapshots.append(_snapshot_from_json(row["payload_json"]))
    ...
    def _read(self, symbol: str, now: float) -> TickerSnapshot | None:
        ...
        try:
            return _snapshot_from_json(row["payload_json"])
        except (TypeError, ValueError, json.JSONDecodeError) as exc:
...
def _snapshot_from_json(payload_json: str) -> TickerSnapshot:
